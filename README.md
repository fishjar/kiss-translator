...
打开网页翻译面板，点击「编辑网站规则」。选择规则用途后点击「选取元素」，在网页上点击锁定元素；通过祖先路径调整层级，比较不同定位候选及匹配数量，最后点击「确认添加规则」保存。选取链接时不会跳转。

- 支持翻译目标、排除区域、限制扫描范围，以及高级的保留原文和段落边界。可以修改、重新选取、删除、撤销和重做。
- 自动识别模式下，目标选择器不是翻译白名单；需要严格指定目标时，请明确切换到「只翻译指定目标」。删除一条定位不等于排除区域，其他规则或自动扫描仍可能覆盖它。
- 「恢复继承」重新使用订阅／全局值；「清空此组」保存显式空选择器。清空根容器后不扫描页面。
- 新规则采用 `hostname:example.com` 格式，仅匹配该主机的 HTTP/HTTPS 页面，包括不同端口，不包含子域名。首次保存会保留当前较宽个人规则的其他配置；订阅内容通过个人规则覆盖。
- 确认修改后立即写入本地。其他页面修改规则时会提示冲突并保留输入，可以重新读取后比较。撤销历史仅保留在当前编辑会话，路由切换或重新读取规则后清空。
- 「预计翻译范围」复用翻译器的目标扫描，只查看当前已加载 DOM，不发送翻译请求；内部排除项、保留项和后续语言／长度过滤仍然生效。「查看译文」会执行实际翻译。退出时恢复进入前的翻译和交互开关。
