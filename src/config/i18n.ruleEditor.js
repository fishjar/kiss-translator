const messages = {
  ...
  auto: ["自动识别并调整范围", "Detect text automatically"],
  manual: ["只翻译指定目标", "Translate selected targets only"],
  mode: ["翻译模式", "Translation mode"],
  modeHelp: [
    "自动模式中目标不是白名单；手动模式只扫描目标。切换将立即保存并改变整体范围。",
    "Targets are not a whitelist in automatic mode. Manual mode scans only targets. Switching saves immediately and changes the overall scope.",
  ],
  selector: ["翻译目标", "Translation targets"],
  ignoreSelector: ["排除区域", "Excluded areas"],
  rootsSelector: ["限制扫描范围", "Scan roots"],
