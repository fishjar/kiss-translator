...
export const I18N = {
  ...
  prompt_management_help: {
    zh: `集中管理翻译、AI 断句和未来 AI 词典会使用的提示词。预设只能复制为模板，自定义提示词可以修改和删除。`,
    en: `Manage prompts used by translation, AI segmentation, and future AI dictionaries. Presets can only be copied as templates; custom prompts can be edited and deleted.`,
    zh_TW: `集中管理翻譯、AI 斷句和未來 AI 詞典會使用的提示詞。預設只能複製為範本，自訂提示詞可以修改和刪除。`,
    ja: `翻訳、AI セグメンテーション、今後の AI 辞書で使うプロンプトをまとめて管理します。プリセットはテンプレートとしてコピーでき、自作プロンプトは編集と削除ができます。`,
    ko: `번역, AI 문장 분할, 향후 AI 사전에 사용할 프롬프트를 한곳에서 관리합니다. 프리셋은 템플릿으로만 복사할 수 있고, 사용자 프롬프트는 수정하고 삭제할 수 있습니다.`,
  },
  prompt_name: {
    zh: `名称`,
    en: `Name`,
  ...
  prompt_category_batch_system: {
    zh: `batch system prompt`,
    en: `batch system prompt`,
    zh_TW: `batch system prompt`,
    ja: `batch system prompt`,
    ko: `batch system prompt`,
  },
