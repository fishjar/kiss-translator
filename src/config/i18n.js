...
export const I18N = {
  ...
  tones_info: {
    zh: `管理 AI 翻译的风格/语气预设。选中的预设会在翻译时附加到系统提示词中。`,
    en: `Manage AI translation tone presets. The selected tone will be appended to the system prompt during translation.`,
    zh_TW: `管理 AI 翻譯的風格/語氣預設。選中的預設會在翻譯時附加到系統提示詞中。`,
    ja: `AI翻訳のトーンプリセットを管理します。選択されたトーンは翻訳時にシステムプロンプトに追加されます。`,
    ko: `AI 번역 톤 프리셋을 관리합니다. 선택한 톤은 번역 시 시스템 프롬프트에 추가됩니다.`,
  },
  tone_name: {
    zh: `名称`,
    en: `Name`,
  ...
  tone_instruction_helper: {
    zh: `此指令会附加到系统提示词中。留空表示不附加额外指示。`,
    en: `This instruction will be appended to the system prompt. Leave empty for no additional instructions.`,
    zh_TW: `此指令會附加到系統提示詞中。留空表示不附加額外指示。`,
    ja: `この指示はシステムプロンプトに追加されます。空のままにすると追加指示はありません。`,
    ko: `이 지시는 시스템 프롬프트에 추가됩니다. 추가 지시가 없으면 비워 두세요.`,
  },
  active_tone: {
    zh: `使用中`,
    en: `Active`,
