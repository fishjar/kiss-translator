...
export const I18N = {
  ...
  about_api_3: {
    zh: `3、对于一些小模型，建议优先选用 单句翻译 (Concise) 或 聚合翻译（LINE）。`,
    en: `3. For smaller models, it is recommended to prioritize Single-sentence Translation (Concise) or Batch Translation (LINE).`,
    zh_TW: `3. 對於一些小型模型，建議優先選用 單句翻譯 (Concise) 或 聚合翻譯（LINE）。`,
    ja: `3. 一部の小型モデルでは、「単文翻訳（Concise）」または「一括翻訳（LINE）」を優先して選択することをお勧めします。`,
    ko: `3. 일부 소형 모델의 경우 '단문 번역 (Concise)' 또는 '일괄 번역 (LINE)'을 우선 사용하는 것을 권장합니다.`,
    tr: `3. Bazı küçük modeller için Tek Cümle Çeviri (Concise) veya Toplu Çeviri (LINE) tercih edilmesi önerilir.`,
    vi: "3. Đối với một số mô hình nhỏ, nên ưu tiên chọn Dịch từng câu (Concise) hoặc Dịch hàng loạt (LINE).",
  },
  about_api_4: {
