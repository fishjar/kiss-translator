...
describe("handleTranslate", () => {
  ...
  // ─── QwenMT 机制分离验证 ─────────────────────────────────────────────────
  // 两种术语机制互不混淆：
  // - translation_options.terms 是 QwenMT 服务端术语能力（只来自 glossary/aiTerms）；
  // - 本地正则替换（rule.terms）发生在序列化阶段（占位符保护），不属于 apiTranslate。
  // 两者同时启用时：本地先替换（原文从待翻译文本消失）→ 服务端 terms 不再命中原文，无重复处理。

  test("QwenMT translation_options.terms 只来自 glossary/aiTerms，本地正则术语不进请求体", async () => {
    fetchData.mockResolvedValueOnce({
      choices: [{ message: { role: "assistant", content: "译文" } }],
    });

    await collectAsyncGenerator(
      handleTranslate(["React API"], {
        from: "en",
        to: "zh-CN",
        fromLang: "English",
        toLang: "Chinese",
    ...
    const body = JSON.parse(fetchData.mock.calls[0][1].body);
    // 服务端 terms = glossary 与 aiTerms 的合并，本地正则术语（rule.terms）不存在于本层
    expect(body.translation_options.terms).toEqual(
      expect.arrayContaining([
        { source: "React", target: "响应框架" },
        { source: "API", target: "应用接口" },
      ])
    );
    // 本地替换是序列化阶段行为：请求内容保持原文（本层不做本地替换）
    expect(body.messages[0].content).toBe("React API");
  });

  test("本地替换先于服务端 terms：原文消失后 QwenMT terms 不重复处理", async () => {
    fetchData.mockResolvedValueOnce({
      choices: [{ message: { role: "assistant", content: "译文" } }],
    });

    // 模拟完整链路的中间状态：本地正则术语已在序列化阶段把 AI 的 "API" 替换为
    // 译文占位后还原的文本（原文 "API" 从待翻译文本中消失）；同一术语同时配置在
    // aiTerms（服务端 terms）中 —— 服务端 terms 因原文不再存在而不命中，无重复替换。
    await collectAsyncGenerator(
      handleTranslate(["请配置好应用接口"], {
        from: "zh-CN",
        to: "en",
        fromLang: "Chinese",
        toLang: "English",
    ...
    const body = JSON.parse(fetchData.mock.calls[0][1].body);
    // 本地已消费原文 → 待翻译文本中不再出现 "API"，服务端 terms 不会二次命中
    expect(body.messages[0].content).toBe("请配置好应用接口");
    expect(body.messages[0].content).not.toContain("API");
    // 服务端 terms 照常携带（机制独立），但原文已不存在 → 无冲突、无重复替换
    expect(body.translation_options.terms).toEqual([
      { source: "API", target: "应用接口" },
    ]);
    expect(body.messages[0].content.split("应用接口")).toHaveLength(2);
  });
  ...
  test("Ollama 流式：空 delta 轮整对不写，round 2 体恰为 [system, u2]、round 3 体含 round 2 完整对", async () => {
    ...
    // E4 修正：此处必须预写多行——单行形态按 prettier 的 East Asian 宽度计法
    // （CJK 记 2）为 82 列 > printWidth 80（.prettierrc 实证），--check 必炸；
    // 多行形态与文件内既有 thirdBody 断言（"第二轮"用例）逐字同构。
